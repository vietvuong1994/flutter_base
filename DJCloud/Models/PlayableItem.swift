...
//		artistObj = try container.decode(Artist?.self, forKey: .artistObj);
    ...
    init(json: JSON) {
        _id = json["id"].int?.string()
        title = json["title"].string
        if let image = json["gallery"].string {
            imageUrl = Constants.API_BASE_URL + image
        }
        var listArtist: [Artist] = []
        var artistName = ""
        json["postable"]["singers"].arrayValue.forEach { (js) in
            let at = Artist(json: js)
            if let name = at.name {
                artistName += name + ","
            }
            listArtist.append(at)
        }
        artist = String(artistName.dropLast())
        artistObj = listArtist
        slug = json["slug"].string
        viewCount = json["view_count"].int
        avgRating = json["avg_rating"].int
    }
}
